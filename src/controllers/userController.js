...
const getapi = async function (req, res) {
  try {
    ...
    if(validUserId!=userId){
        return res.status(401).send({status:false,message:"user don't have permission to get details"})
    }
...
const logInUser = async function (req, res) {
 ...
 const {email,password}=req.body
    ...
    if (!check)
      return res
        .status(400)
        .send({ status: false, msg: "incorrect email" });
    ...
    res.status(200).send({ status: true, message: "User login successfull", data: {userId:check._id,token:token} });
  } catch (err) {
    res.status(500).send({ status: false, msg: err.message });
  }
};

module.exports = { userCreate, getapi, logInUser };
