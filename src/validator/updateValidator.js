...
const updateValidatior = async function (req, res , next) {
    ...
    if (email) {
        ...
        if (uniqueEmail) {
            return res.status(400).send({ status: false, message: `${email} is   already Used try another email` })
        }
        final.email = email
    }
    ...
    if (phone) {
        ...
        if (uniquePhone) {
            return res.status(400).send({ status: false, message: `${phone} is  already Used try another phoneNumber` })
        }
        final.phone = phone
    }
